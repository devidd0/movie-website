import React, { useEffect, useState } from "react";
import { useParams } from "react-router";
import { MdKeyboardArrowRight } from "react-icons/md";
import { Helmet } from "react-helmet";
const FilmDetail = () => {
  const id = useParams();
  const [filmDetail, setFilmDetail] = useState();
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/${id.id}?api_key=a447a294be2eab4dd49113efb54fb2d3&language=tr-TR`
    )
      .then((res) => res.json())
      .then((data) => setFilmDetail(data));
  }, [id]);
  console.log(filmDetail);
  return (
    <div className="bg-[#333] h-auto min-h-screen">
      <Helmet>
        <title>{filmDetail && filmDetail.title}</title>
      </Helmet>
      <div className=" lg:p-10 p-5 lg:flex-row flex flex-col items-start ">
        <div className="image lg:w-[40rem] block mb-2 mx-auto w-[16rem]  border-4 rounded overflow-hidden flex-shrink-0 relative max-h-96">
          <img
            src={
              filmDetail && filmDetail.poster_path === null
                ? "data:image/png;base64,"
                : `https://image.tmdb.org/t/p/w500/${
                    filmDetail && filmDetail.backdrop_path
                  }`
            }
            alt={filmDetail && filmDetail.title}
            className="w-full"
          />
          <div className="absolute right-3 bottom-3 bg-yellow-500 px-4 rounded text-white text-lg">
            {filmDetail && filmDetail.vote_average}
          </div>
        </div>
        <div className="text-white ml-4">
          <div className="pb-2 break-words flex text-start lg:w-full w-[17rem] border-b-2 leading-7 text-base lg:text-lg">
            <p className="first-letter:capitalize first-letter:font-bold first-letter:text-lg">
              {(filmDetail && filmDetail.overview) ||
                "Bu Filmin Bir aciqlamasi yoxdur"}
            </p>
          </div>

          <div className="actions mt-6 flex justify-between">
            <a
              href={filmDetail && filmDetail.homepage}
              target="_blank"
              className="flex flex-shrink-0 items-center font-semibold border-2 rounded w-28 px-2  h-14 justify-between group hover:bg-indigo-600"
            >
              İndi İzlə
              <MdKeyboardArrowRight className="font-bold text-2xl group-hover:translate-x-1 transition-all" />
            </a>

            <div className="flex items-center flex-wrap mx-12">
              {filmDetail &&
                filmDetail.genres.map((genre, key) => (
                  <div key={key} className="border px-2 m-1 rounded">
                    {genre.name}
                  </div>
                ))}
            </div>
          </div>
        </div>
      </div>
      <div className="border-t-4 flex flex-col p-8">
        <h2 className="font-bold text-white text-3xl mb-6 mx-auto">
          Film Haqqında əlavə məlumatlar
        </h2>
        <ul className="flex w-full justify-around lg:flex-row flex-col">
          <li className="text-white border-2 rounded px-4 py-2 mx-2 lg:my-0 my-4 hover:scale-105 transition-all text-center">
            <div className="font-semibold text-lg">Film Büddəsi</div>{" "}
            {filmDetail && filmDetail.budget.toLocaleString()}
          </li>
          <li className="text-white border-2 rounded px-4 py-2 mx-2 lg:my-0 my-4 hover:scale-105 transition-all text-center">
            <div className="font-semibold text-lg">Film Dili</div>{" "}
            <span className="uppercase font-bold">
              {filmDetail && filmDetail.original_language}
            </span>
          </li>
          <li className="text-white border-2 rounded px-4 py-2 mx-2 lg:my-0 my-4 hover:scale-105 transition-all text-center">
            <div className="font-semibold text-lg">Film Buraxılış ili</div>{" "}
            <span className="uppercase font-bold">
              {filmDetail && filmDetail.release_date}
            </span>
          </li>
          <li className="text-white border-2 rounded px-4 py-2 mx-2 hover:scale-105 transition-all text-center">
            <div className="font-semibold text-lg">Film Uzunluğu</div>{" "}
            <span className="uppercase font-bold flex items-center justify-center">
              {filmDetail &&
                `${Math.floor(filmDetail.runtime / 60)}:${
                  filmDetail.runtime % 60
                }`}
              <div className="ml-1">Saat</div>
            </span>
          </li>
        </ul>
      </div>
      <div className="border-t-4 flex flex-col p-8">
        <h2 className="font-bold text-white text-3xl mb-6 mx-auto">
          Danşılan Dillər
        </h2>
        <ul>
          {filmDetail &&
            filmDetail.spoken_languages.map((lang, key) => (
              <li
                key={key}
                className="my-4 text-white border-2 rounded px-4 py-2 mx-2 transition-all text-center"
              >
                {lang.english_name}
              </li>
            ))}
        </ul>
      </div>
    </div>
  );
};

export default FilmDetail;
